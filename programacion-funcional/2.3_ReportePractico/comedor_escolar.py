...
#    R:Es una funcion que puede ser tratada como cualquier otro objeto
#    o variable, se le puede asignar a variables, almacenar en la estructura de datos,
#    tambien como argumentos para otras funciones y retornadas por ota¿ras funciones.
#
# 2. ¿Cuál es la diferencia entre una funcion de orden superior y un callback?
#    R: Una "funcion de orden superior" es aquella que recibe una funcion como 
#       argumento o devuelve una funcion como resultado, un "callback", por otro 
#       lado, es la función específica que se pasa como argumento para que sea 
#       ejecutada dentro de esa función de orden superior.
#
# 3. ¿Cuándo conviene usar comprensión de listas en lugar de un ciclo for?
#    R: Conviene usar comprensión de listas cuando necesitamos crear una nueva lista
#       transformando o filtrando un iterable de forma sencilla, hace el codigo 
#       más conciso, legible y, a menudo, más rápido, sin embargo, 
#       si la lógica interna es muy compleja o anidada, un ciclo for tradicional 
#       es mejor para mantener la legibilidad.
#
# 4. ¿Qué hace map() y cómo se relaciona con lambda?
#    R:  map() aplica una funcion a cada elemento de un iterable (lista, tupla, etc.)
#      y devuelve un iterable con los resultados.
#     lambda permite crear funciones pequeñas y anónimas en una sola linea.
#      Se suelen usar juntos porque lambda es útil para definir rapidamente
#      la funcion que map() necesita.
#
# 5. ¿Qué ventaja ofrece pasar una funcion como argumento a otra funcion?
#    R: Permite reutilizar codigo y hacerlo más flexible,
#    Hace que las funciones sean más genéricas y adaptables,
#    Facilita la programación funcional (menos repetición de codigo),
#    Permite personalizar el comportamiento sin modificar la funcion original.
# ─────────────────────────────────────────────────────────────────────────────
...
#      - preparar_pizza(), preparar_agua(), preparar_tamal()  ->funciones simples, 0 parametros
#      - calcular_promocion(cantidad)                         -> logica condicional
#      - tomar_orden(preparar_alimento, cantidad, precio_unitario)

#      - ¿Cuál de ellas es de orden superior? ¿Por qué?
#  tomar_orden(), porque recibe preparar_alimento como argumento (es un callback).

#      - ¿Dónde usarás comprensión de listas?
#      En tomar_orden(), para generar la lista de porciones llamando a preparar_alimento().

#      - ¿Dónde usarás lambda + map()?
#   En tomar_orden(), para mapear el precio_unitario a cada elemento de porciones.
# ─────────────────────────────────────────────────────────────────────────────
...
def tomar_orden(preparar_alimento, cantidad, precio_unitario):
    ...
    # llama al callback en cada iteración
    porciones = [preparar_alimento() for _ in range(cantidad)]  # ← reemplaza con list comprehension

    # b) map() + lambda para precios
    # asigna el precio unitario a cada porción
    precios = list(map(lambda porcion: precio_unitario, porciones)) # ← reemplaza con list(map(lambda ..., porciones))
...
#    R= Nada ya que el programa funciona de manera correcta segun los casos de prueba
# ─────────────────────────────────────────────────────────────────────────────
...
#
#
# ─── SOLUCION DESAFIO EXTRA: ───────────────────────────────────────────────────────────

# 1. Total a pagar por orden
...
# 2. Función de orden superior que DEVUELVE otra función
...
# 3. Reemplaza los argumentos directos usando elegir_producto()
